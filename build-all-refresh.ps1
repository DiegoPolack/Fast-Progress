...
$uploadDir = Join-Path $root "_upload"
...
if (Test-Path (Join-Path $root ".gitignore")) {
    Copy-Item -Path (Join-Path $root ".gitignore") -Destination $uploadDir -Force
}
if (Test-Path (Join-Path $root "build-all.ps1")) {
    Copy-Item -Path (Join-Path $root "build-all.ps1") -Destination $uploadDir -Force
}
if (Test-Path (Join-Path $root "build-all-refresh.ps1")) {
    Copy-Item -Path (Join-Path $root "build-all-refresh.ps1") -Destination $uploadDir -Force
}

$excludeDirs = @("build", ".gradle", "run", "logs", "crash-reports", "server", "config", ".idea", ".vscode", ".settings", "builds", "_upload")
$excludeFiles = @("*.iml", "*.log", "*.tmp")

foreach ($dir in $versionDirs) {
    $dest = Join-Path $uploadDir $dir.Name
    robocopy $dir.FullName $dest /E /XD $excludeDirs /XF $excludeFiles | Out-Null
}

Write-Host "Done. Output in $outDir and refreshed $uploadDir"
